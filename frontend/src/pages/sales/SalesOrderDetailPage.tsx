...
export default function SalesOrderDetailPage() {
  ...
  const { data: order, isLoading } = useSalesOrder(ulid ?? '')
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            {order.status === 'draft' && <button className="btn-primary" onClick={() => confirmMut.mutate()} disabled={confirmMut.isPending}><CheckCircle className="w-4 h-4" /> Confirm</button>}
