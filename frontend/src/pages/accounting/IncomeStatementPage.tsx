  ...
  return (
    <div className={`space-y-0.5 ${indent ? 'pl-4' : ''}`}>
      {section.accounts.map((acct, i) => (
        <div key={i} className="flex justify-between items-center py-1 text-sm text-gray-700 px-2">
          <span>
...
function SummaryRow({ label, amount, highlight = false }: {
  ...
  return (
    <div className={`flex justify-between items-center px-2 py-2 ${highlight
      ? 'bg-indigo-50 font-bold text-indigo-900 rounded-lg border border-indigo-200'
      : 'border-t border-gray-200 font-semibold text-gray-800'
    }`}>
      <span>{label}</span>
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Revenue, expenses and net income waterfall (GL-004)
        </p>
      </div>
      ...
      {is && (
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3">
          ...
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide pt-2">Cost of Goods Sold</h2>
          <ISSection section={is.cogs} indent />
          <SummaryRow label="Gross Profit" amount={is.gross_profit} />

          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide pt-2">Operating Expenses</h2>
          <ISSection section={is.operating_expenses} indent />
          <SummaryRow label="Operating Income" amount={is.operating_income} />

          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide pt-2">Tax</h2>
