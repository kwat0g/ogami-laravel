...
import { useArAgingReport, type ArAgingRow } from '@/hooks/useAR'

const BUCKET_LABELS: { key: keyof ArAgingRow; label: string; color: string }[] = [
...
export default function ArAgingReportPage(): React.ReactElement {
  const [asOfDate, setAsOfDate] = useState(new Date().toISOString().split('T')[0])
  const { data, isLoading } = useArAgingReport(asOfDate)

  const rows: ArAgingRow[] = useMemo(() => data?.data ?? [], [data?.data])
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-neutral-500">
            Receivable balances grouped by customer and days overdue.
          </p>
        </div>
        <div>
      ...
      {rows.length > 0 && totals.total > 0 && (
        <div className="mb-6 bg-white border border-neutral-200 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-sm font-medium text-neutral-700">Total Outstanding:</span>
      ...
      <div className="bg-white border border-neutral-200 rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <p className="text-sm text-neutral-500 p-6">No outstanding receivables as of this date.</p>
        ) : (
          <table className="w-full text-sm">
            <thead className="border-b border-neutral-200 bg-neutral-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-neutral-500">Customer</th>
            ...
            <tbody className="divide-y divide-neutral-100">
              {rows.map((row) => (
                <tr key={row.customer_id} className="hover:bg-neutral-50 transition-colors">
                  <td className="px-4 py-3 text-neutral-800 font-medium">{row.customer_name}</td>
