...
import { usePayrollRun, usePayrollApprovals, usePayrollBreakdown, useVpApprovePayroll } from '@/hooks/usePayroll'
  ...
  return (
    <div className="max-w-4xl space-y-6">
        ...
        <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded p-4">
          ...
          <div>
            ...
            <p className="text-xs text-red-700 mt-0.5">
              You initiated this payroll run. A different VP must provide final approval (Segregation of Duties policy).
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-neutral-200 rounded overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-5">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-neutral-900">{formatPHP((run.gross_pay_total_centavos ?? 0) / 100)}</p>
          </div>
          <div>
            <p className="text-xs text-neutral-500">Total Deductions</p>
            <p className="text-lg font-bold text-red-600">{formatPHP((run.total_deductions_centavos ?? 0) / 100)}</p>
          </div>
          <div>
            <p className="text-xs text-neutral-500">Total Net Pay</p>
            <p className="text-lg font-bold text-green-700">{formatPHP((run.net_pay_total_centavos ?? 0) / 100)}</p>
          </div>
        </div>
      </div>
              ...
              <div key={apr.id} className="flex items-center gap-3 text-sm">
                ...
                <div>
                  ...
                  <span className="text-neutral-500 ml-1">
                    ({apr.stage}) — {apr.action.toLowerCase()} on {new Date(apr.acted_at).toLocaleString('en-PH')}
                  </span>
                  {apr.comments && <p className="text-xs text-neutral-400 mt-0.5 italic">"{apr.comments}"</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {breakdown?.data && breakdown.data.length > 0 && (
        <div className="bg-white border border-neutral-200 rounded overflow-hidden">
          <div className="px-5 py-4 border-b border-neutral-100">
            <h3 className="text-sm font-semibold text-neutral-800">Employee Preview (Top {Math.min(5, breakdown.meta?.total ?? 0)} of {breakdown.meta?.total ?? 0})</h3>
          </div>
          <div className="p-5 overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {breakdown.data.map((detail: { id: number; employee?: { first_name: string; last_name: string }; employee_id: number; gross_pay_centavos?: number; net_pay_centavos?: number }) => (
                  <tr key={detail.id} className="border-b border-neutral-100">
                    ...
                    <td className="py-2 text-right font-medium">{formatPHP((detail.net_pay_centavos ?? 0) / 100)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="flex items-center justify-end pt-4 border-t border-neutral-100">
        {isReadOnly ? (
          ...
          <button
            ...
            disabled={vpApprove.isPending || !allChecked || sodViolation || run.status !== 'ACCTG_APPROVED'}
          ...
          >
            {vpApprove.isPending
              ...
              : <><ShieldCheck className="h-4 w-4" /> Approve &amp; Authorize Disbursement <ArrowRight className="h-4 w-4" /></>
            }
          </button>
        )}
      </div>
