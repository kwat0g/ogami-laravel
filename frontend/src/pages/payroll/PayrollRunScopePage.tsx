          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-5">
            ...
            <div className="space-y-3">
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-sm text-gray-700">Include employees on unpaid leave</span>
                <button
                  type="button"
                  role="switch"
                  ...
                  className={`relative inline-flex h-5 w-9 rounded-full transition-colors ${
                    includeUnpaidLeave ? 'bg-blue-600' : 'bg-gray-300'
                  }`}
                >
                  <span className={`inline-block h-4 w-4 bg-white rounded-full shadow transform transition-transform mt-0.5 ${
                    includeUnpaidLeave ? 'translate-x-4' : 'translate-x-0.5'
                  }`} />
                </button>
              </label>
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-sm text-gray-700">Include employees whose probation ends this period</span>
                <button
                  type="button"
                  role="switch"
                  ...
                  className={`relative inline-flex h-5 w-9 rounded-full transition-colors ${
                    includeProbationEnd ? 'bg-blue-600' : 'bg-gray-300'
                  }`}
                >
                  <span className={`inline-block h-4 w-4 bg-white rounded-full shadow transform transition-transform mt-0.5 ${
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4 sticky top-6">
                  ...
                  <div className="space-y-1 max-h-64 overflow-y-auto">
                    ...
                    {preview.by_department?.map(dept => (
                      <div key={dept.department_id} className="flex items-center justify-between text-sm py-1 border-b border-gray-50">
                        ...
                        <span className="text-blue-600 font-medium shrink-0">{dept.in_scope} <span className="text-gray-400 font-normal">/ {dept.eligible}</span></span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <button
              ...
              className="w-full px-4 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
            >
          ...
          >
            ...
            <div className="flex justify-end gap-2 pt-1">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-1"
              >
