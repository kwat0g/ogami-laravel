...
import {
  ArrowLeft, ArrowRight, ShieldAlert, ThumbsUp, XCircle, Loader2, Eye,
} from 'lucide-react'
import { usePayrollRun, useGlPreview, useAcctgApprove, usePayrollApprovals, usePayrollBreakdown } from '@/hooks/usePayroll'
          ...
          <tbody>
            {data.data.map((detail: { id: number; employee?: { first_name: string; last_name: string; employee_code?: string }; employee_id: number; gross_pay_centavos?: number; sss_ee_centavos?: number; philhealth_ee_centavos?: number; pagibig_ee_centavos?: number; tax_withheld_centavos?: number; net_pay_centavos?: number }) => (
              <tr key={detail.id} className="border-b border-neutral-100">
                ...
                <td className="py-2 text-right">{formatPHP((detail.philhealth_ee_centavos ?? 0) / 100)}</td>
                <td className="py-2 text-right">{formatPHP((detail.pagibig_ee_centavos ?? 0) / 100)}</td>
                <td className="py-2 text-right">{formatPHP((detail.withholding_tax_centavos ?? 0) / 100)}</td>
...
export default function PayrollRunAcctgReviewPage() {
  ...
  // Prevent going back if payroll is already in accounting review or beyond
  const canGoBack = !['SUBMITTED', 'HR_APPROVED', 'ACCTG_APPROVED', 'APPROVED', 'POSTED', 'DISBURSED'].includes(run.status)
        ...
        <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded p-4">
          ...
          <div>
            ...
            <p className="text-xs text-red-700 mt-0.5">
              You initiated this payroll run. A different user must perform Accounting approval (Segregation of Duties policy).
            </p>
          </div>
        </div>
      )}
