    ...
    queryFn: async () => {
      const res = await api.get<{ data: ScopePreview }>('/payroll/runs/draft-scope-preview', {
        params: {
          ...
          exclude_employee_ids:    params.exclude_employee_ids?.length ? params.exclude_employee_ids : undefined,
        },
      })
...
export default function PayrollRunDraftScopePage() {
  ...
  const [employmentTypes,    setEmploymentTypes]    = useState<string[]>(
    state.step2?.employment_types ?? ['regular', 'probationary', 'contractual', 'project_based', 'casual'],
  )
  const [departments,        _setDepartments]       = useState<number[]>(state.step2?.departments ?? [])
  const [includeUnpaidLeave,  setIncludeUnpaidLeave]  = useState(state.step2?.include_unpaid_leave  ?? false)
  const [includeProbationEnd, setIncludeProbationEnd] = useState(state.step2?.include_probation_end ?? false)
  ...
  function handleAddExclusion() {
    ...
    setExclusions(prev => [
      ...prev,
      { employee_id: selectedEmployee.id, label: selectedEmployee.full_name, reason: exclReason.trim() },
    ])
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-5">
            ...
            <div className="space-y-3">
              {([
                [includeUnpaidLeave,  setIncludeUnpaidLeave,  'Include employees on unpaid leave'] as const,
