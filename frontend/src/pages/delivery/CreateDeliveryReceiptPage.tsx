  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">New Delivery Receipt</h1>
          <p className="text-sm text-gray-500 mt-0.5">Record inbound or outbound goods movement</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
          <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Receipt Details</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="date"
                className={`w-full border rounded-lg px-3 py-2 text-sm ${receiptDateError ? 'border-red-400' : 'border-gray-300'}`}
                value={receiptDate}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={addRow}
              className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-teal-600 hover:bg-teal-700 text-white font-medium rounded-lg"
            >
            ...
            {lineItems.map((row, idx) => (
              <div key={idx} className="grid grid-cols-12 gap-2 items-start bg-gray-50 rounded-lg p-3">
                ...
                <div className="col-span-4">
                  {idx === 0 && <p className="text-xs text-gray-500 mb-1">Item *</p>}
                  <select
                ...
                <div className="col-span-1">
                  {idx === 0 && <p className="text-xs text-gray-500 mb-1">Expected</p>}
                  <input
                    type="number"
                    step="0.001"
                    min="0"
                ...
                {/* Received Qty */}
                <div className="col-span-1">
                  {idx === 0 && <p className="text-xs text-gray-500 mb-1">Received</p>}
                  <input
                    type="number"
                    step="0.001"
                    min="0"
                ...
                {/* Lot/Batch */}
                <div className="col-span-2">
                  {idx === 0 && <p className="text-xs text-gray-500 mb-1">Lot/Batch</p>}
                  <input
                    type="text"
                    className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm"
                ...
                {/* Remarks */}
                <div className="col-span-1">
                  {idx === 0 && <p className="text-xs text-gray-500 mb-1">Note</p>}
                  <input
                    type="text"
                    className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm"
                    value={row.remarks}
