    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'shipments' => $this->whenLoaded('shipments', fn () => $this->shipments->map(fn ($s) => [
                ...
                'estimated_arrival' => $s->estimated_arrival?->toDateString(),
                'shipped_at' => $s->shipped_at?->toISOString(),
                'actual_arrival' => $s->actual_arrival?->toDateString(),
                'notes' => $s->notes,
            ])),
            'shipments_count' => $this->whenCounted('shipments'),
            'vehicle_id' => $this->vehicle_id,
            'driver_name' => $this->driver_name,
            'delivery_schedule_id' => $this->delivery_schedule_id,
            'pod_receiver_name' => $this->pod_receiver_name,
            'pod_photo_urls' => $this->pod_photo_urls ?? [],
            'pod_notes' => $this->pod_notes,
            'pod_recorded_at' => $this->pod_recorded_at?->toIso8601String(),
