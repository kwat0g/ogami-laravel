...
return new class extends Migration
{
    ...
    private function seedSystemTemplates(): void
    {
        ...
        $salariesAccount = ChartOfAccount::where('code', '6100')->first();
        $sssPayable = ChartOfAccount::where('code', '2105')->first();
        $philhealthPayable = ChartOfAccount::where('code', '2106')->first();
        $pagibigPayable = ChartOfAccount::where('code', '2107')->first();
        $withholdingTaxPayable = ChartOfAccount::where('code', '2108')->first();
        $cashAccount = ChartOfAccount::where('code', '1100')->first();
        $depreciationExpense = ChartOfAccount::where('code', '6500')->first();
        $accumulatedDepreciation = ChartOfAccount::where('code', '1501')->first();
        $loanPayable = ChartOfAccount::where('code', '2200')->first();
        ...
        if ($salariesAccount && $cashAccount) {
            ...
            if ($sssPayable) {
                $lines[] = ['account_id' => $sssPayable->id, 'debit_or_credit' => 'credit', 'description' => 'SSS contributions payable'];
            }
            if ($philhealthPayable) {
                $lines[] = ['account_id' => $philhealthPayable->id, 'debit_or_credit' => 'credit', 'description' => 'PhilHealth contributions payable'];
            }
            if ($pagibigPayable) {
                $lines[] = ['account_id' => $pagibigPayable->id, 'debit_or_credit' => 'credit', 'description' => 'Pag-IBIG contributions payable'];
            }
            if ($withholdingTaxPayable) {
                $lines[] = ['account_id' => $withholdingTaxPayable->id, 'debit_or_credit' => 'credit', 'description' => 'Withholding tax payable'];
            }
        ...
        if ($apAccount) {
            ...
            $assetAccount = ChartOfAccount::where('code', 'like', '15%')->first();
            if ($assetAccount) {
                $templates[] = [
